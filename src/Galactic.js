class Galactic{
  ...
  lifeExpOnVenus() {
    if(this.earthAge > this.averageLife) {
        let surpassedYears = (this.earthAge - this.averageLife)*.62;
        return Math.floor(surpassedYears);
    } else {
        let remainingYears = this.averageLife - this.earthAge;
        return Math.floor(remainingYears*.62);
    }
  }
  lifeExpOnMars() {
    if(this.earthAge > this.averageLife) {
  ...
  lifeExpOnJp() {
    if(this.earthAge > this.averageLife) {
        let surpassedYearsOnJp = (this.earthAge - this.averageLife)*11.86;
        return Math.floor(surpassedYearsOnJp);
    } else {
        let remainingYears = this.averageLife - this.earthAge;
        return Math.floor(remainingYears*11.86);
    }
  }
